...
pred_latents = np.load('cache/predicted_embeddings/BIGMEG1/thingsmeg_regress_autokl_sub-BIGMEG1.npy')
ref_latent = stats
...
for i in range(int(np.ceil(len(test_images)/batch_size))):
  ...
  for j in range(len(sample_from_latent)):
      ...
      subject = 'BIGMEG1'
      save_dir = 'results/vdvae/' + subject + '/'
      if not os.path.exists(save_dir):
          os.makedirs(save_dir)
      im.save('results/vdvae/{}/{}.png'.format(subject,i*batch_size+j))
