...
# %%
train_clip_vision = None
test_clip_vision = None
for session, task, run in tqdm(product(sessions, tasks, runs), total=len(sessions)*len(tasks)*len(runs), desc=f'Processing subject {subject}'):
    ...
    train_image_ids = pd.to_numeric(train_events_df['image_id']).astype(int).to_numpy() - 1 # -1 if we need one-based indexing
    test_image_ids = pd.to_numeric(test_events_df['image_id']).astype(int).to_numpy() - 1 # -1 if we need one-based indexing
...
print('train_clip_vision shape: ', train_clip_vision.shape) # (22248, 257, 768)
print('test_clip_vision shape: ', test_clip_vision.shape) # (2400, 257, 768)

# %%
save_dir = 'cache/extracted_embeddings/' + subject + '/'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
np.save(save_dir + f'train_clipvision1b_sub-{subject}.npy', train_clip_vision)
np.save(save_dir + f'test_clipvision1b_sub-{subject}.npy', test_clip_vision)
