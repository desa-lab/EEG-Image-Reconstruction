...
for session, task, run in tqdm(product(sessions, tasks, runs), total=len(sessions)*len(tasks)*len(runs), desc=f'Processing subject {subject}'):
    ...
    if train_stim_ids is None:
        train_stim_ids = pd.to_numeric(train_events_df['image_id']).astype(int).to_numpy() # +1 if we need one-based indexing
        test_stim_ids = pd.to_numeric(test_events_df['image_id']).astype(int).to_numpy() # +1 if we need one-based indexing
    else:
        train_stim_ids = np.concatenate((train_stim_ids, pd.to_numeric(train_events_df['image_id']).astype(int).to_numpy()), axis=0)
        test_stim_ids = np.concatenate((test_stim_ids, pd.to_numeric(test_events_df['image_id']).astype(int).to_numpy()), axis=0)
...
np.save(save_dir + f'train_images_sub-{subject}.npy', train_images)
np.save(save_dir + f'test_images_sub-{subject}.npy', test_images)
np.save(save_dir + f'train_text_sub-{subject}.npy', train_text)
np.save(save_dir + f'test_text_sub-{subject}.npy', test_text)
